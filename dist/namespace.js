...
export function checkQName(name) {
    const colon = name.indexOf(":");
    if (!/^[A-Za-z_][\w\.\xB7-]*(?::[A-Za-z_][\w\.\xB7-]*|)$/.test(name)) {
        throw DOMException.new("InvalidCharacterError", `Name '${name}'`);
    }
}
export function validateAndExtract(ns, qualifiedName) {
    if (ns) {
        let prefix, localName, pos = qualifiedName.indexOf(":");
        if (pos >= 0) {
            prefix = qualifiedName.substring(0, pos);
            localName = qualifiedName.substring(pos + 1);
            if (!/^[_A-Za-z]\w*:[_A-Za-z][\w_-]*$/.test(qualifiedName)) {
                throw DOMException.new("InvalidCharacterError", `qualifiedName '${qualifiedName}'`);
            }
        }
        else {
            prefix = "";
        ...
        if ((prefix && !ns) ||
            ...
            throw DOMException.new("NamespaceError", `NS[${ns}] QN[${qualifiedName}] LN[${localName}] P[${prefix}]`);
        }
        ...
        if (qualifiedName.indexOf(":") < 0) {
            if (!NameRE.test(qualifiedName)) {
                throw DOMException.new("InvalidCharacterError", `name '${qualifiedName}'`);
            }
            return [null, null, qualifiedName];
        }
        else {
            throw DOMException.new("NamespaceError", `'${ns}' '${qualifiedName}'`);
        }
    }
}

...
test(function () {
  ...
  for (var i = 0; i < invalid_names.length; i++) {
    assert_throws_dom("INVALID_CHARACTER_ERR", function () {
      el.toggleAttribute(invalid_names[i], true);
    });
  }
  for (var i = 0; i < invalid_names.length; i++) {
    assert_throws_dom("INVALID_CHARACTER_ERR", function () {
      el.toggleAttribute(invalid_names[i]);
    });
  }
  for (var i = 0; i < invalid_names.length; i++) {
    assert_throws_dom("INVALID_CHARACTER_ERR", function () {
      el.toggleAttribute(invalid_names[i], false);
    });
  }
...
test(function () {
  ...
  for (var i = 0; i < invalid_names.length; i++) {
    assert_throws_dom("INVALID_CHARACTER_ERR", function () {
      el.setAttribute(invalid_names[i], "test");
    });
  }
}, "When qualifiedName does not match the Name production, an " +
...
test(function () {
  var el = document.createElement("foo");
  assert_throws_dom("NAMESPACE_ERR", function () {
...
test(function () {
  ...
  assert_equals(
    el.hasAttributeNS(undefined, "A"),
    true,
    "el.hasAttributeNS(undefined)"
  );
  assert_equals(
    el.hasAttributeNS("foo", "A"),
    false,
    'el.hasAttributeNS("foo")'
  );

  assert_equals(el.getAttribute("A"), "test", "getAttribute()");
  assert_equals(el.getAttributeNS("", "A"), "test", 'el.getAttributeNS("")');
  assert_equals(
    el.getAttributeNS(null, "A"),
    "test",
    "el.getAttributeNS(null)"
  );
  assert_equals(
    el.getAttributeNS(undefined, "A"),
    "test",
    "el.getAttributeNS(undefined)"
  );
  assert_equals(
    el.getAttributeNS("foo", "A"),
    null,
    'el.getAttributeNS("foo")'
  );
...
test(function () {
  ...
  var attrNode = el.getAttributeNode("foo");
  var el2 = document.createElement("div");
  assert_throws_dom("INUSE_ATTRIBUTE_ERR", function () {
    el2.setAttributeNode(attrNode);
  });
...
test(function () {
  ...
  assert_array_equals(
    Array.prototype.map.call(el.attributes, function (a) {
      return a.name;
    }),
    ["a", "c"]
  );
...
function getEnumerableOwnProps2(obj) {
  return Object.getOwnPropertyNames(obj).filter(function (name) {
    return Object.getOwnPropertyDescriptor(obj, name).enumerable;
  });
}
...
test(function () {
  ...
  assert_array_equals(getEnumerableOwnProps1(el.attributes), ["0", "1"]);
  assert_array_equals(getEnumerableOwnProps2(el.attributes), ["0", "1"]);
...
test(function () {
  ...
  assert_array_equals(getEnumerableOwnProps1(el.attributes), ["0", "1", "2"]);
  assert_array_equals(getEnumerableOwnProps2(el.attributes), ["0", "1", "2"]);
  // assert_array_equals(Object.getOwnPropertyNames(el.attributes),
  //                     ["0", "1", "2", "a", "b"])
  for (var propName of Object.getOwnPropertyNames(el.attributes)) {
    assert_true(
      el.attributes[propName] instanceof Attr,
